...
func (c *PGWriter) setupPgPrometheus() error {
	...
	_, err = c.DB.Exec(context.Background(), "CREATE OR REPLACE VIEW network_metrics AS select v.metric_id, l.metric_name,l.metric_labels ->> 'job' as job, l.metric_labels ->> 'device' as device, l.metric_labels ->> 'instance' as instance, v.metric_time, v.metric_value from metric_labels l, metric_values v where l.metric_id= v.metric_id and l.metric_name in ('node_network_carrier_up_changes_total','node_network_carrier_down_changes_total','node_network_receive_bytes_total','node_network_transmit_bytes_total','node_network_transmit_errs_total','node_network_transmit_packets_total');")
	if err != nil {
		return err
	}

	_, err = c.DB.Exec(context.Background(), "CREATE OR REPLACE VIEW disk_metrics AS select v.metric_id, l.metric_name,l.metric_labels ->> 'job' as job, l.metric_labels ->> 'device' as device, l.metric_labels ->> 'instance' as instance, v.metric_time, v.metric_value from metric_labels l, metric_values v where l.metric_id= v.metric_id and l.metric_name in ('node_disk_read_bytes_total','node_disk_written_bytes_total', 'node_disk_io_now');")
	if err != nil {
		return err
	}

	_, err = c.DB.Exec(context.Background(), "CREATE OR REPLACE VIEW cpu_metrics AS select v.metric_id, l.metric_name,l.metric_labels ->> 'job' as job, l.metric_labels ->> 'cpu' as cpu, l.metric_labels ->> 'mode' as mode, l.metric_labels ->> 'instance' as instance, v.metric_time, v.metric_value from metric_labels l, metric_values v where l.metric_id= v.metric_id and l.metric_name in ('node_cpu_seconds_total');")
	if err != nil {
		return err
	}
